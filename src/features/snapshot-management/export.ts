...
import { JSZipAsync } from '@/shared/lib/chunk';
...
import { getImageId } from '@/entities/snapshot/urls';
import { detectRemoteSnapshot } from '@/entities/snapshot/api';
...
export const exportSnapshotAsImageId = async (snapshot: Snapshot) => {
  return (
    ...
    uploadAsset(
      ...
      snapshotMetadataActions.setImageId(snapshot.id, imageId);
      return imageId;
    })
  );
};
...
export const exportSnapshotAsImportId = async (snapshot: Snapshot) => {
  return (
    ...
    uploadAsset(
      ...
      snapshotMetadataActions.setImportId(snapshot.id, r.id);
      snapshotMetadataActions.setSnapshotId(r.id, snapshot.id);
