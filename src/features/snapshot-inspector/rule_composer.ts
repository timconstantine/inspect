...
export const createRuleComposerDefaults = (now = Date.now()) => ({
  groupKey: 1,
  groupName: `[ChangeMe]RuleName-${dayjs(now).format('YYYY-MM-DD HH:mm:ss')}`,
  groupDescription: '[ChangeMe]Generated by the GKD web inspector',
  preKeys: [] as number[],
  includeLimits: false,
  matchRoot: false,
...
export const parsePreKeys = (
  ...
  if (!items.every((item) => /^-?\d+$/u.test(item))) {
    return { values: [], error: 'preKeys must be comma-separated integers' };
  }
  const values = items.map(Number);
  if (!values.every(Number.isSafeInteger)) {
    return {
      values: [],
      error: 'preKeys contains an integer outside the safe range',
    };
  }
...
export const createRulePosition = (
  ...
  if (!(target.width > 0) || !(target.height > 0)) {
    throw new Error(
      `Target node has no valid size, can't generate relative coordinates`,
    );
  }
  return {
